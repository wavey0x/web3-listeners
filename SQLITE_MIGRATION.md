...
Status: production cutover and notification activation completed on `electro` on 10 September 2026; the 24-hour observation completed on 11 September. The listeners use `/var/lib/yearn/yearn.sqlite3` with SQLite 3.53.4. Live progress, notification permissions, hourly backups, and the deployed test suite were verified again on 12 September. The bundle requires an explicit worker selection and validates prepared state before starting workers. The rehearsal sections below preserve the original migration evidence; their pending cutover steps have since completed.

## Live alert latency (12 September 2026)

DAO, Curve votes, and retention changes scan `latest` with a two-second idle poll. Eligible alerts are attempted after their SQLite transaction commits, without waiting for confirmations or finality. DAO status changes and reminders use the latest indexed block timestamp, and activation requires catch-up to the observed latest head. Receipt decoding, RPC response time, database commits, and Telegram still add processing time; two seconds is the poll interval, not a delivery guarantee.

RSUP and YieldBasis reports still need a completed week and the first calculation block after its boundary. They use the latest head and wake at week close, polling every two seconds once that boundary is due. They do not wait an extra hour or for finality. Harvest indexing has no notification path and continues using finalized blocks. Inactive import boundary adoption also continues to require finality.

There is no automatic reorg recovery, rollback, or alert correction. Existing block/receipt/checkpoint consistency checks stop a mismatched scan for manual reconciliation. Atomic data/checkpoint commits and durable notification deduplication remain in place. Restart and explicit activation still set a latest-head notification floor, suppressing historical/outage alerts; delivery attempts with uncertain outcomes are not replayed.

The 12 September investigation traced proposal 29's execution in block 25,961,434 at 12:48:35 UTC. The previous finalized-only scanner delivered it at 13:04:23 UTC, a 15m48s delay. The live paths now use the latest head to remove that wait. The suite has 83 tests, including alerts before finality, no duplicate after finalization, DAO time-based transitions, and incentive boundary scheduling. Worker scan ranges, notification session floors, and delivery outcomes are logged without credentials.
...
Run `--prepare-import` and then `--adopt-boundary` on `data_fetchers/curve_gauge_votes.py` against the inactive imported copy. Normal startup does neither. The live listener scans latest mainnet blocks and commits votes, event identities, notification decisions, and scan progress together. A failed insert or RPC cannot advance the range or send a notice. Empty ranges advance progress. A changed saved block hash pauses the listener for reconciliation.
...
During the coordinated cutover, keep the shared alert flag and stream disabled until validation passes. After silent catch-up, the activation procedure creates the root-controlled file, enables shared notification eligibility, and invokes `--enable-alerts`. That command requires a validated scan through the observed latest head and sets its notification floor there, so events already present at activation are ineligible. Future eligible events may be announced as soon as they are indexed. `--mute-alerts` disables the stream and invalidates its current session. A machine-level hold must also remove its activation file and stop the service and its activation sources; removing a file does not terminate an already running process or undo a request already in flight.
...
The `retention` stream uses the same durable suppression, latest-head startup floor, and single-attempt transport as Curve, retaining the `RESUPPLY_ALERTS` destination. Its root-controlled activation file is `retention.allow`. Standalone `--enable-alerts` requires validated silent catch-up through the latest head; `--mute-alerts` invalidates the session. The bundle's `main()` entry point performs no argument parsing, and imports perform no database, RPC, or notification work.
...
Normal scans wait for the entire week and its calculation block to exist on chain. RPC log requests are bounded, period boundaries exclude adjacent weeks, and all calculations finish before a short write transaction. Incentive rows, stable event identities, notification decisions, and completed-week progress commit together. Empty weeks also advance. A failed gauge lookup, malformed transfer, database write, or changed block hash leaves the period unfinished; it cannot silently publish partial gauge data or move past a failed week. An unavailable optional token price still produces null efficiency values, matching the existing behavior. Existing floating-point and JSON representations are preserved.
